...
function task_timeout() -- kick users not sending ping event in 2 minutes
  ...
  for k,v in pairs(users) do
    ...
    if tmpdata.pings >= max_pings then
      vRP.kick(v,"[vRP] You Have Timed Out Due To Not Pinging The Server")
    end
  end
...
AddEventHandler("playerConnecting",function(name,setMessage, deferrals)
  ...
  if ids ~= nil and #ids > 0 then
    deferrals.update("[vRP] Checking IDs")
    vRP.getUserIdByIdentifiers(ids, function(user_id)
      ...
      if user_id ~= nil then -- check user validity 
        deferrals.update("[vRP] Checking Banned State")
        vRP.isBanned(user_id, function(banned)
          if not banned then
            deferrals.update("[vRP] Checking If You Have Sufficent Whitelisting")
            vRP.isWhitelisted(user_id, function(whitelisted)
              if not config.whitelist or whitelisted then
                ...
                if vRP.rusers[user_id] == nil then -- not present on the server, init
                  ...
                  deferrals.update("[vRP] Loading Database Tables")
                  vRP.getUData(user_id, "vRP:datatable", function(sdata)
                    ...
                    deferrals.update("[vRP] Getting Player Information From ID")
                ...
                deferrals.done("[vRP] You Are Not Whitelisted (user_id = "..user_id..").")
              end
            end)
          else
